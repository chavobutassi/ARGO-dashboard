...
def actualizar_global_precios(_i, _c):
    if not GLOBAL_DISPONIBLE:
        return [_lbl("Precios globales"),
                html.Div("No disponible", style={"fontSize": "12px",
                                                  "color": COLORES["texto2"]})], ""
    try:
    ...
    try:
        ts_label = f"Actualizado {datetime.fromisoformat(ts).strftime('%H:%M')}"
    except Exception:
        ts_label = ""
    ...
    for clave, nombre in granos:
        ...
        filas.append(html.Div(style={
            ...
            html.Div(style={"display": "flex", "alignItems": "center", "gap": "10px"},
                ...
                html.Div(f"{p['tendencia']} {p['cambio_pct']:+.1f}%", style={
                    "fontSize": "11px", "fontWeight": "500",
                    "color": p["color"], "minWidth": "60px", "textAlign": "right",
                }),
            ]),
        ]))

    # Ratio soja/maíz
    soja_p = precios.get("soja", {}).get("precio", 0)
    maiz_p = precios.get("maiz", {}).get("precio", 0)
    ratio_div = []
    if soja_p and maiz_p:
        ratio = round(soja_p / maiz_p, 2)
        color_ratio = COLORES["BAJO"] if ratio > 2.3 else COLORES["MEDIO"]
        ratio_div = [html.Div(style={
            "marginTop": "10px", "padding": "8px 10px",
            "backgroundColor": COLORES["fondo"],
            "borderRadius": "6px", "display": "flex",
            "justifyContent": "space-between", "alignItems": "center",
        }, children=[
            html.Div("Relación soja / maíz", style={
                "fontSize": "11px", "color": COLORES["texto2"]}),
            html.Div(f"{ratio}x", style={
                "fontSize": "13px", "fontWeight": "600", "color": color_ratio}),
        ])]
...
def actualizar_global_señales(_i, _c):
    if not GLOBAL_DISPONIBLE:
        return []
    try:
    ...
    impacto_color = {"ALTO": COLORES["CRITICO"], "MEDIO": COLORES["MEDIO"],
                     "BAJO": COLORES["BAJO"]}
    items = []
    for s in señales[:6]:
        color = s.get("color", COLORES["azul"])
        badge_c = impacto_color.get(s.get("impacto", "MEDIO"), COLORES["MEDIO"])
        items.append(html.Div(style={
            ...
            html.Div(style={
                ...
                html.Div(style={"width": "8px", "height": "8px",
                                ...
                                "flexShrink": "0"}),
                html.Div(s["titulo"], style={"fontSize": "12px", "fontWeight": "600",
                                             "color": COLORES["texto"], "flex": "1"}),
                html.Div(s.get("categoria", ""), style={
                    "fontSize": "9px", "color": COLORES["texto2"],
                    "border": f"0.5px solid {COLORES['borde']}",
                    "borderRadius": "4px", "padding": "1px 6px",
                }),
                html.Div(s.get("impacto", ""), style={
                    "fontSize": "9px", "fontWeight": "500", "color": badge_c,
                    "border": f"0.5px solid {badge_c}",
                    "borderRadius": "4px", "padding": "1px 6px",
                }),
            ]),
...
def actualizar_global_ranking(_i):
    if not GLOBAL_DISPONIBLE:
        return []
    try:
    ...
    for r in ranking[:10]:
        ...
        filas.append(html.Div(style={
            ...
            html.Div(f"{r['pct_global']}%", style={
                "fontSize": "11px", "fontWeight": "500", "minWidth": "40px",
                "textAlign": "right",
                "color": COLORES["BAJO"] if es_arg else COLORES["texto2"],
            }),
        ]))
    ...
    return [
        ...
        *filas,
        html.Div("Fuente: FAO / campaña 2022-23", style={
            "fontSize": "10px", "color": COLORES["texto2"], "marginTop": "8px",
        }),
    ]
...
def actualizar_global_usda(_i):
    ...
    def _stat(label, valor, unidad="MM tn"):
        return html.Div(style={
            ...
            html.Div(f"{valor} {unidad}" if valor not in ("N/D", None) else "N/D",
                     style={"fontSize": "12px", "fontWeight": "600",
                            "color": COLORES["texto"]}),
        ])

    return [
        _lbl("Oferta y demanda mundial — Soja · Maíz · Trigo"),
        html.Div(style={
            "backgroundColor": COLORES["fondo_medio"],
            "border": f"1px solid {color_señal}",
            "borderRadius": "8px", "padding": "10px 14px", "marginBottom": "12px",
        }, children=[
            html.Div(f"Stocks soja: {señal}", style={
                "fontSize": "12px", "fontWeight": "600",
                "color": color_señal, "marginBottom": "4px",
            }),
            html.Div(desc, style={
                "fontSize": "11px", "color": COLORES["texto2"], "lineHeight": "1.5",
            }),
        ]),
        _stat("Producción global soja",  usda.get("produccion_global_mm_tn", "N/D")),
        _stat("Stocks finales soja",     usda.get("stocks_finales_mm_tn", "N/D")),
        _stat("Ratio stocks/uso soja",   usda.get("ratio_stocks_uso_pct", "N/D"), "%"),
        _stat("Producción global maíz",  usda_maiz.get("produccion_global_mm_tn", "N/D")),
        _stat("Producción global trigo", usda_trigo.get("produccion_global_mm_tn", "N/D")),
        html.Div("Fuente: USDA PSD / campaña actual", style={
            "fontSize": "10px", "color": COLORES["texto2"], "marginTop": "8px",
        }),
    ]
